...
qwq 是一款功能丰富的浏览器扩展，旨在提升您的日常浏览体验。它集成了多种实用工具，从页面截图、IP 地址查询到高级的 DeepSeek 对话背景自定义和图像编辑功能，以及更多便捷操作。我们致力于提供一个强大、稳定且易于使用的工具集，让您的浏览器更加个性化和高效。

qwq is a feature-rich browser extension designed to enhance your daily browsing experience. It integrates a variety of practical tools, ranging from page screenshots and IP address lookups to advanced DeepSeek chat background customization and image editing features, and many more convenient operations. We are committed to providing a powerful, stable, and easy-to-use toolkit to make your browser more personalized and efficient.
...
qwq 扩展已上架到 [微软 Edge 扩展中心](https://microsoftedge.microsoft.com/addons/detail/your-extension-id-here) (请替换为实际的扩展 ID)。

您可以通过访问上述链接，点击“获取”按钮即可轻松安装。

qwq is available on the [Microsoft Edge Add-ons store](https://microsoftedge.microsoft.com/addons/detail/your-extension-id-here) (please replace with your actual extension ID).
...
5.  点击“应用”按钮保存并应用设置。设置将自动保存，并在您下次访问 `chat.deepseek.com` 时生效。
...
## 未来版本兼容性 (Future Compatibility)

本 README 文件旨在提供一个全面的概述，并尽可能地保持通用性，以适应未来的版本更新。核心功能和安装说明预计将保持稳定。对于新增功能，我们可能会在 README 中添加简要说明，但主要细节将通过代码注释或更详细的文档提供。

This README file aims to provide a comprehensive overview and remain as generic as possible to accommodate future version updates. Core functionalities and installation instructions are expected to remain stable. For new features, we may add brief descriptions to the README, but primary details will be provided through code comments or more detailed documentation.
